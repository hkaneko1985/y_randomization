...
"""
# Demonstration of y-randamization
...
number_of_yrandamization = 30
max_number_of_pls_components = 20
fold_number = 5

# generate sample dataset
x, y = datasets.make_regression(n_samples=number_of_training_samples+number_of_test_samples, n_features=number_of_x_variables, n_informative=10, noise=30, random_state=0)
...
# y-randamization
statistics_yrand = np.empty([number_of_yrandamization, 6])
for y_rand_num in range(number_of_yrandamization):
    print('{0} / {1}'.format(y_rand_num+1, number_of_yrandamization))
    ...
    for pls_component in pls_components:
        ...
        calculated_y_in_cv = np.ndarray.flatten(pls_model_in_cv.predict(autoscaled_x_train))
        estimated_y_in_cv = np.ndarray.flatten(
            model_selection.cross_val_predict(pls_model_in_cv, autoscaled_x_train, autoscaled_y_train_rand, cv=fold_number))
        calculated_y_in_cv = calculated_y_in_cv * y_train.std(ddof=1) + y_train.mean()
        estimated_y_in_cv = estimated_y_in_cv * y_train.std(ddof=1) + y_train.mean()
    ...
    estimated_y_in_cv_rand = np.ndarray.flatten(
            model_selection.cross_val_predict(regression_model, autoscaled_x_train, autoscaled_y_train_rand, cv=fold_number))
    estimated_y_in_cv_rand = estimated_y_in_cv_rand * y_train.std(ddof=1) + y_train.mean()
    
    statistics_yrand[y_rand_num, 0] = float(1 - sum((y_train_rand - estimated_y_train_rand) ** 2) / sum((y_train_rand - y_train.mean()) ** 2))
    ...
    statistics_yrand[y_rand_num, 3] = float(1 - sum((y_train_rand - estimated_y_in_cv_rand) ** 2) / sum((y_train_rand - y_train.mean()) ** 2))
...
plt.plot(np.ones(number_of_yrandamization), statistics_yrand[:, 0], 'b.')
plt.plot(np.ones(number_of_yrandamization) * 2, statistics_yrand[:, 3], 'b.')
plt.xticks([1, 2],['r2rand', 'r2rand,cv'])
plt.ylabel('r2')
plt.show()
print('average: {0}, {1}'.format(statistics_yrand[:, 0].mean(), statistics_yrand[:, 3].mean()))

plt.plot(np.ones(number_of_yrandamization), statistics_yrand[:, 1], 'b.')
plt.plot(np.ones(number_of_yrandamization) * 2, statistics_yrand[:, 2], 'r.')
plt.plot(np.ones(number_of_yrandamization) * 3, statistics_yrand[:, 4], 'b.')
plt.plot(np.ones(number_of_yrandamization) * 4, statistics_yrand[:, 5], 'r.')
plt.xticks([1, 2, 3, 4],['RMSErand', 'MAErand', 'RMSErand,cv', 'MAErand,cv'])
plt.ylabel('RMSE(blue), MAE(red)')
plt.show()
print('average: {0}, {1}, {2}, {3}'.format(statistics_yrand[:, 1].mean(), statistics_yrand[:, 2].mean(), statistics_yrand[:, 4].mean(), statistics_yrand[:, 5].mean()))
